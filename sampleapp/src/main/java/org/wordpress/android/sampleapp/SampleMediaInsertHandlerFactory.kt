...
class SampleMediaInsertHandlerFactory @Inject constructor(
    ...
    override fun build(mediaPickerSetup: MediaPickerSetup): MediaInsertHandler {
        return when (mediaPickerSetup.primaryDataSource) {
            DEVICE, CAMERA, SYSTEM_PICKER -> {
                deviceMediaInsertUseCaseFactory.build(mediaPickerSetup.areResultsQueued)
            }
